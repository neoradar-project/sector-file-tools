import { Color, Position, toGeoJson } from '../src';
    ...
    it('converts fix to a Point feature', function () {
        const output = toGeoJson({
            ...
            fixes: [
                {
                    id: 'BAVAD',
                    position: Position.latlon('N060.27.57.999', 'E011.05.03.998'),
                },
            ],
        }, emptyEse, null, true, 'WGS84');
        expect(output.features).to.eql([
...
describe('Use ASR to determine which features to include in GeoJSON output', function () {
    it('picks only the ARTCCs defined in the ASR', function () {
        const result = toGeoJson({
            ...
            artccLow: makeDummyArtccs([
                'ARTCC-LOW-1',
                'ARTCC-LOW-2',
                'ARTCC-LOW-3',
            ]),
            artccHigh: makeDummyArtccs([
                'ARTCC-HIGH-1',
                'ARTCC-HIGH-2',
                'ARTCC-HIGH-3',
            ]),
        ...
        const unique = [
            ...new Set(result.features.map((feature) => feature.properties.section)),
        ];
