...
export function toGeoJson(
  ...
  const features: Feature[] = flatten([
    sct.regions
      .filter((region) =>
        asr != null ? asr.regions.includes(region.id) : true
      )
    ...
    sct.artccLow
      ...
      .flatMap((artcc) => geoToGeo(artcc, 'artccLow', system)),
    sct.artccHigh
      .filter((artcc) =>
        asr != null ? asr.artccHigh.includes(artcc.id) : true
      )
      .flatMap((artcc) => geoToGeo(artcc, 'artccHigh', system)),
    sct.highAirway.flatMap((airway) => geoToGeo(airway, 'highAirway', system)),
    sct.lowAirway.flatMap((airway) => geoToGeo(airway, 'lowAirway', system)),
  ]);
